...
def setup_and_verify():
    ...
    verify_arguments_and_set_cli_options()
    load_config()
...
def verify_arguments_and_set_cli_options():
    opts = {}
    args=[]
...
def verify_release_base_and_release_branch_exist():
    if(options.hotfix):
        ...
        git('rev-parse', '--quiet', '--verify', state.release_base(), error="Base revision not found!")
